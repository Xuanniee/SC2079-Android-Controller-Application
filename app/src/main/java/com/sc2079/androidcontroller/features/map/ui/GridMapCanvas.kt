...
fun GridMapCanvas(
    ...
    val textSizePx = with(density) { 11.sp.toPx() }
    val axisTextSizePx = with(density) { 10.sp.toPx() }
    val targetTextSizePx = with(density) { 20.sp.toPx() }
    ...
    Canvas(
        ...
        clipPath(gridClipPath) {
            ...
            uiState.obstacles.forEach { obs ->
                ...
                obs.displayedTargetId?.let { tid ->
                    val thick = rect.width * 0.18f
                    val c = targetColor(tid)

                    when (obs.faceDir) {
                        FaceDir.NORTH -> drawLine(
                            color = c,
                ...
                drawContext.canvas.nativeCanvas.apply {
                    ...
                    // Number the obstacle only in White Paint
                    val tid = obs.displayedTargetId
                    if (tid != null) {
                        paint.textSize = targetTextSizePx
                        paint.color = android.graphics.Color.WHITE
                        drawText("$tid", cx, cy + targetTextSizePx * 0.35f, paint)
                    } else {
...
private fun targetColor(targetId: Int): Color {
    return when ((targetId % 10 + 10) % 10) {
        0 -> Color(0xFFEF5350)
        1 -> Color(0xFFAB47BC)
        2 -> Color(0xFF5C6BC0)
        3 -> Color(0xFF29B6F6)
